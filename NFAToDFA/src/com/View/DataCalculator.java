...
public class DataCalculator {
	...
	private void getData(String file) throws IOException {
		...
		selfLoops = new SelfLoop[self_loop_count];
			...
			for (Entry<String, HashSet<State>> e : automataStates[i].transitions.entrySet()) {
				for (State state : e.getValue()) {
					...
					if (i == state.stateID) {
						...
						selfLoops[sidx] = new SelfLoop(sidx, p, radius, theta);
						selfLoops[sidx].string_pos = getPointWRT(states[i].centre, Math.toRadians(states[i].angle),
								1.5 * Circle.RADIUS, 1, 1);
						selfLoops[sidx].end = q2;

						theta = calTheta(selfLoops[sidx].end, states[i].centre);
						q1 = selfLoops[sidx].end;
						q2 = states[i].centre;
						int Xm = 1, Ym = 1;
						...
						selfLoops[sidx].p1 = getPointRotation(q, selfLoops[sidx].end, 80);
						selfLoops[sidx].p2 = getPointRotation(q, selfLoops[sidx].end, 350);
						selfLoops[sidx].input_symbol = input[i][state.stateID];
						sidx++;
						continue;
					}
					Arrow a = new Arrow(tidx);
					q1 = states[i].centre;
					q2 = states[state.stateID].centre;
					...
					a.input_symbol = input[i][state.stateID];
